...
public class RoundedDrawable extends Drawable {
    ...
    private boolean mIsDisplayText = false ;
    ...
    public RoundedDrawable(Bitmap bitmap, boolean isDisplayText, String text) {
        mBitmap = bitmap;
        mIsDisplayText = isDisplayText;
        mText = text;
    ...
    public static Drawable fromDrawable(Drawable drawable, boolean isDisplayText, String text) {
        if (drawable != null) {
            if (drawable instanceof RoundedDrawable) {
                // just return if it's already a RoundedDrawable
                return drawable;
                ...
                for (int i = 0; i < num; i++) {
                    Drawable d = ld.getDrawable(i);
                    ld.setDrawableByLayerId(ld.getId(i), fromDrawable(d, isDisplayText, text));
                }
                return ld;
            }
            ...
            Bitmap bm = drawableToBitmap(drawable);
            if (bm != null) {
                return new RoundedDrawable(bm, isDisplayText, text);
            }
        }
        return drawable;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (mOval) {
            if (mBorderWidth > 0) {
                ...
                canvas.drawOval(mDrawableRect, mBitmapPaint);
                if (mIsDisplayText) {
                    canvas.drawText(mText,  mDrawableRect.width()/2 - mTextPaint.measureText(mText)/2 , mDrawableRect.height()/2 + mTextHeight + 2,
                            mTextPaint);
                }
            }
        } else {
